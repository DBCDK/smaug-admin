...
export default function AdgangsplatformForm(props) {
  ...
  const checkForServices = (type) => {
    return jsonConfig.services ? jsonConfig.services.filter(t => t === type).length > 0 : false;
  };
  const checkForAttributes = (type) => {
    return jsonConfig.attributes ? Object.keys(jsonConfig.attributes).filter(t => t === type).length > 0 : false;
  };
  const checkForIdProviders = (type) => {
    return jsonConfig.identityProviders ? jsonConfig.identityProviders.filter(t => t === type).length > 0 : false;
  };
  ...
  const getredirectUris = () => {
    let incomingURIs = isJSON(jsonConfig.redirectUris) ? jsonConfig.redirectUris : JSON.stringify(jsonConfig.redirectUris, null, 2);
    return incomingURIs;
  };

  const [openPlatform, setOpenplatform] = useState(checkForServices('OpenPlatform'));
  ...
  const [muniNo, setMuniNo] = useState(checkForAttributes('municipalityAgencyId'));
  ...
  const getServices = () => {
    ...
    openPlatform ? arr.push('OpenPlatform') : arr.filter(e => e === 'OpenPlatform');
    return arr;
  };
  const getAttributes = () => {
  ...
  const saveTextToJSON = (e, fn) => {
    ...
    // send
//    console.log("OUT",(JSON.parse(jsonOUT["redirectUris"]))
  ...
  return (
    <div className="adgangsForm-line">
        <span className="adgangsForm-title-top">
            <div className="adgangsForm-title"><label>Services</label></div>
            <div className="adgangsForm-toggle" onClick={toggleJson}>Edit configuration as json EDITS</div>
          </span>
      <div className="adgangsForm">
        <div className="adgangsForm-choosePlatform-container" id="services">
          ...
          <span className="openPlatform">
              <input id="openPlatform" type="checkbox" checked={openPlatform} onChange={() => setOpenplatform(!openPlatform)} />
              <input-label>OpenPlatform</input-label>
            </span>
          <span className="bib">
              <input id="bib" type="checkbox" checked={bib} onChange={() => setBib(!bib)} />
        ...
        <div className="adgangsForm-formSection">
          <div className="adgangsForm-title-top">
            <div className="adgangsForm-title"><label>Configure login.bib.dk</label></div>
          </div>

          <div className="adgangsForm-formBg">

            <div className="input-label">Agency Id</div>
            <input id="agencyId" type="text" defaultValue={agencyId} placeholder="Type library no of agency" onChange={e => saveTextToJSON(e, setAgencyId)}
                   required="required" />
            <div className="input-label">Logo color</div>
            <input id="logoColor" type="text" defaultValue={logoColor} placeholder="Type color as HEX value" onChange={e => saveTextToJSON(e, setLogoColor)} />

            { /* User attributes at sign in */}
            <div className="adgangsForm-checkbox-section" id="attributes">
              <input-label>User attributes to retrieve at sign in</input-label>
              <span className="userCPR">
                  <input id="userCPR" type="checkbox" checked={userCPR} onChange={() => setUserCPR(!userCPR)} />
                  <input-label>User CPR</input-label>
                </span>
              <span className="userID">
                  <input id="userID" type="checkbox" checked={userID} onChange={() => setUserID(!userID)} />
                  <input-label>User ID (retrieved from the library, which is selected at sign in)</input-label>
                </span>
              <span className="uniqueID">
                  <input id="uniqueID" type="checkbox" checked={uniqueID} onChange={() => setUniqueID(!uniqueID)} />
                  <input-label>Unique ID (from CULR)</input-label>
                </span>
              <span className="libraries">
                  <input id="libraries" type="checkbox" checked={libraries} onChange={() => setLibraries(!libraries)} />
                  <input-label>Libraries</input-label>
                </span>
              <span className="muniNo">
                  <input id="muniNo" type="checkbox" checked={muniNo} onChange={() => setMuniNo(!muniNo)} />
                  <input-label>Municipality no</input-label>
                </span>
            </div>
            <div className="input-label">Display name</div>
            <input id="displayName" type="text" defaultValue={displayName} placeholder="Type name to display to user" onChange={e => saveTextToJSON(e, setDisplayName)}
                   required="required" />

            { /* Identity providers  */}
            <div className="adgangsForm-checkbox-section" id="identityProviders">
              <input-label>Identity providers</input-label>
              <span>
                  <input id="borchk" type="checkbox" checked={borchk} onChange={() => setBorchk(!borchk)} />
                  <input-label>Borchk</input-label>
                </span>
              <span>
                  <input id="nemlogin" type="checkbox" checked={nemLogin} onChange={() => setNemLogin(!nemLogin)} />
                  <input-label>NemLogin</input-label>
                </span>
              <span>
                  <input id="unilogin" type="checkbox" checked={uniLogin} onChange={() => setUniLogin(!uniLogin)} />
            ...
            <div className="adgangsForm-checkbox-section" id="logoutScreen">
              ...
              <span>
                  <input id="showSignOut" type="checkbox" checked={!!showSignOut} onChange={() => setShowSignOut(!showSignOut)} />
                  <input-label> Show sign out screen to user, when signing out</input-label>
                </span>
            </div>

            <div className="input-label"> Single sign out path</div>
            <input id="singleLogoutPath" type="text" defaultValue={singleSignoutPath} placeholder="Type URL to redirect to, when user signs out"
                   onChange={e => saveTextToJSON(e, setSingleSignoutPath)} />

            <div className="input-label">Redirect URIs</div>
            <textarea id="redirectUris" defaultValue={redirectUris} placeholder=" Type allowed URLs which the user can be redirected to, when signing out"
